...
class CustomEnv(gym.Env):
    def __init__(self, training=True, inputIndex=2, qos=29):
        ...
        self.action_space = gym.spaces.Discrete(2) # totally 8 possible actions for the agent
    ...
    def placementInit(self):
        ...
        try:
            ...
            faceanalyzerfn_command = ['faas', 'deploy', '-f', 'functions.yml', '--filter=faceanalyzerfn', '--constraint']
            subprocess.check_call(faceanalyzerfn_command, cwd='../../functions/version1', stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
            mobilenetfn_command = ['faas', 'deploy', '-f', 'functions.yml', '--filter=mobilenetfn', '--constraint']
    ...
    def reset(self):
        ...
        framerfn_pos, facedetectorfn2_pos, models_pos, number_of_facedetector, number_of_models = self.getPositions(self)
...
model = DQN("MlpPolicy", env, policy_kwargs=policy_kwargs, verbose=1, train_freq=(1, "step"), learning_rate=0.0025, learning_starts=15,
            ...
            exploration_initial_eps=1, exploration_final_eps=0.01, tensorboard_log="./logs/kube/%s/" % dt)
