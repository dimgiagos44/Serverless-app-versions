...
check_start_command = 'kubectl logs gateway-7ff44f68cb-hv4zt gateway -n openfaas --since-time=' + start_now_str + '| grep -e "/function/mobilenetfn" -e "/function/faceanalyzerfn" | tail -n 1 | wc -l'
while (command_received < 1):
    command_received = int(subprocess.getoutput(check_start_command))

started = True
count_frames_command = 'kubectl logs gateway-7ff44f68cb-hv4zt gateway -n openfaas --since-time=' + start_now_str + '| grep -e "/function/mobilenetfn" -e "/function/faceanalyzerfn" | tail -n' + str(frames_number) + ' | wc -l'
